...
class phpSimpleMailer
{
    ...
    public function receiveConnect() {
        ...
        $this->imapServer = '{' . $this->mailServer . ':' . $this->imapPort . '/imap/ssl}INBOX';

        $this->marubox=@imap_open($this->imapServer, $this->Username, $this->Password);
     ...
     * @Author Bruce Vim < qrfvim@163.com >
     */
    public function getUnseenMails($day=7)
    {
        if(!$this->marubox)
            return false;
        ...
        //先清除一下缓存 防止手动标未读的获取不到
        $this -> exeImapGc();

        return imap_sort($this->marubox, SORTDATE, 1,0,$search_criteria);
    }
     ...
     * @Author Bruce Vim < qrfvim@163.com >
     */
    public function exeImapGc($cache=IMAP_GC_ELT){
        if(!$this->marubox)
            return false;

        return imap_gc($this->marubox, $cache);
    }
     ...
     * @Author Bruce Vim < qrfvim@163.com >
     */
    public function closeReceiveMailbox()
    {
        if(!$this->marubox)
            return false;
     ...
     * @Author Bruce Vim < qrfvim@163.com >
     */
    public function moveMails($msglist,$mailbox)
    {
        if(!$this->marubox)
            return false;

        return imap_mail_move($this->marubox, $msglist, $mailbox);
    }
     ...
     * @return bool|object
     */
    function getImapStatus($option=SA_UNSEEN) {
        if(!$this->marubox)
            return false;
        return imap_status($this->marubox, $this->imapServer,$option);
    }
    ...
    function getHeaders($mid)
    {
        ...
        {
            $subject=$this->decodeMime($mail_header->subject);

            $fromList=array();
            foreach ($mail_header->from as $k => $v)
            {
                $fromList[]=$v->mailbox.'@'.$v->host;
            }
            $toList=array();
            ...
            $ccList=implode(",", $fromList);
            $toList=implode(",", $toList);
            $mail_details=array(
                'fromBy'    => strtolower($sender->mailbox).'@'.$sender->host,
                'fromName'  => $this->decodeMime($sender->personal),
                'ccList'    => $ccList,
                'toNameOth' => $this->decodeMime($sender_replyto->personal),
                'subject'   => $subject,
     ...
     * @Author Bruce Vim < qrfvim@163.com >
     */
    function getMineType(&$structure)
    {
    ...
    function getAttach($mid,$path='')
    {
        ...
        if($struckture->parts)
        {
            foreach($struckture->parts as $key => $value)
            {
                ...
                {
                    //命名附件,转码
                    $name   = $this->decodeMime($struckture->parts[$key]->dparameters[0]->value);
                    $extend = explode("." , $name);
                ...
                if(isset($struckture->parts[$key]->parts))
                {
                    foreach($struckture->parts[$key]->parts as $keyb => $valueb)
                    {
                        ...
                        {
                            //命名图片
                            $name=$this->decodeMime($struckture->parts[$key]->parts[$keyb]->dparameters[0]->value);
                            $extend =explode("." , $name);
    ...
    function getBody($mid,$path='',$imageList)
    {
        ...
        $body = $this->getPart($this->marubox, $mid, "TEXT/HTML");
        if ($body == "")
            $body = $this->getPart($this->marubox, $mid, "TEXT/PLAIN");
        if ($body == "") {
            return "";
        }
    ...
    function getPart($stream, $msg_number, $mime_type, $structure = false, $part_number = false)
    {
        ...
        if($structure) {
            if($mime_type == $this->getMineType($structure))
            {
                if(!$part_number)
                {
                    $part_number = "1";
     ...
     * @Author Bruce Vim < qrfvim@163.com >
     */
    function embedImages(&$body,$path='',$imageList)
    {
        // get all img tags
...
// 			if (!isset($arr['host']) || !isset($arr['path']))continue;
     ...
     * @Author Bruce Vim < qrfvim@163.com >
     */
    function setFlagMails($mid, $status='\\Seen')
    {
        if(!$this->marubox)
            return false;

        return imap_setflag_full($this->marubox, $mid, $status);
    }
